namespace GithubIssueTrackerApp.Models
{
    ...
    public class Issue
    {
        ...
        {
            var priority = string.Format("Priority: {0}", this.GetPriorityAsString());

            var issue = new StringBuilder();
            issue.AppendLine(this.Title).AppendFormat(priority).AppendLine().AppendLine(this.Description);
